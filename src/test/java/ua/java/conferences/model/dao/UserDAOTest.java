...
class UserDAOTest {
    ...
    void testRegisterForEvent() throws DAOException {
        ...
        Assertions.assertTrue(userDAO.isRegistered(ID_VALUE, ID_VALUE));
    }
    ...
    void testBadRegistrations2() throws DAOException {
        ...
        Assertions.assertFalse(userDAO.isRegistered(ID_VALUE, ID_VALUE));
    }
    ...
    @Test
    void testGetParticipants() throws DAOException {
        eventDAO.add(getTestEvent());
        List<User> randomUsers = getRandomUsers();
        for (User user : randomUsers) {
            userDAO.add(user);
            userDAO.registerForEvent(user.getId(), getTestEvent().getId());
        }
        userDAO.add(getTestUser());
        assertIterableEquals(randomUsers, userDAO.getParticipants(getTestEvent().getId(), Role.VISITOR));
    }

    @Test
    void testGetParticipantsSpeakers() throws DAOException {
        eventDAO.add(getTestEvent());
        List<User> speakers = getSpeakers();
        for (int i = 0; i < 3; i++) {
            Report testReport = getTestReport();
            testReport.setSpeaker(speakers.get(i));
            reportDAO.add(testReport);
        }
        assertIterableEquals(speakers, userDAO.getParticipants(getTestEvent().getId(), Role.SPEAKER));
    }
